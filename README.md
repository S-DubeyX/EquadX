...
users/ └─ userId/ ├─ fullName ├─ email ├─ role (student/admin) ├─ points └─ scans/ └─ binId/ ├─ lastScanDate ├─ points └─ timestamp

bins/ └─ binId/ ├─ active ├─ points ├─ lat └─ lng
...
Student scans QR → location + validity checked
Points rewarded instantly
...
Add your google_maps_api_key in AndroidManifest.xml
Connect Firebase project (Authentication + Firestore)
Run on physical device (recommended for camera & maps)
⚠ Notes
